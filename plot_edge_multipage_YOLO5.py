...
def get_nodes(bounding_boxes, labels_yolo, page, num_arch_node, class3):
    ...
    for index_i in range(len(bounding_boxes)):
        ...
        if index_i > 0 and page[index_i] == page[index_i-1] :
            ...
            blu_plot_new = True
        while k < 15 and index_j-k >= 0 : #index_j < len(bounding_boxes):
            ...
            if page[index_j-k] == page[index_i]: 
               # print(distances,labels_yolo[index_j-k] , labels_yolo[index_i] )
                if blu_plot==True and condition_edge(bounding_boxes, labels_yolo, index_j-k, index_i, distances):
                    ...
                    if labels_yolo[index_j-k] in ['sec', 'para']:
                       blu_plot = False   
                    count_edge = count_edge+1
                    
            elif labels_yolo[index_j-k] != 'meta':#if labels_yolo[index_j] != 'meta' and labels_yolo[index_j] != 'tab':
                # se ho : meta, tab, fig 
                if blu_plot_new and condition_edge_page(count_element_page, labels_yolo, index_j-k, index_i, page, distances):
                    j_node.append(index_i)
                    i_node.append(index_j-k)
                    labels.append(1)
                    count_edge = count_edge+1 
                    blu_plot_new = False  
                    #break
                elif class3==True and blu_plot_new == True and title_condition(labels_yolo, index_i, index_j-k, distances): # Quello successivo
                        j_node.append(index_i)
                        i_node.append(index_j-k)
                        labels.append(2)
                        blu_plot_new = False
                    ...
                    if labels_yolo[index_i] in ['sec', 'para']:
                       blu_plot_new = False   
...
# Cy 
def title_condition(labels_yolo, s, t, distances):
    return (labels_yolo[s] in ['sec', 'para'] and 
                                     labels_yolo[t] in ['para', 'sec'])


def condition_edge_page(count_element_page, labels_yolo, index_i, index_j, page, distances):
    if labels_yolo[index_j] == labels_yolo[index_i]:
        return True
    return False
...
def condition_edge(bounding_boxes, labels_yolo, index_i, index_j, distances):
    ...
    #stessa colonna o colonne diverse
    return (0<(distances) < 10 and labels_yolo[index_i]== labels_yolo[index_j]) \
...
def get_color(name_class):
    if name_class == 'sec1' or name_class == 'sec'  or name_class == 'title':
        class_lab = 'red'
        ...
        class_lab = 'magenta'
    elif name_class == 'is_meta' or name_class == 'meta':
        class_lab = 'green' 
    else:
        class_lab = 'black'
    return class_lab
...
def get_graph_yolo(kr, num_arch_node, class3, type_data):
    ...
    path_image = 'zexp_yolo_9_hrdh/images/'#top_folder + 'images/'
